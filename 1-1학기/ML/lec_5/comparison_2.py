...
title_ols = f"DLS: Exact Solution \nw: {ols.coef_[0]:.2f}, b: {ols.intercept_:.2f}"
plt.title(title_ols, fontsize=12, fontweight='bold')

plt.subplot(1,3,2)
plt.scatter(X_s,y,color='orange', alpha=0.5)
plt.plot(X, sgd_short.predict(X_s), color='red', linestyle= '--', label = '1 Iteration')
...
plt.subplot(1,3,3)
plt.scatter(X_s,y,color='orange', alpha=0.5)
plt.plot(X, sgd_long.predict(X_s), color='green', linewidth=3, label='1000 Iterations')
