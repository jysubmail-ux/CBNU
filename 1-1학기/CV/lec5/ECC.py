...
img_show = np.hstack((img, labelmap.astype(np.float32) / (num_labels - 1)))

cv2.imshow('Connected components', img_show)
cv2.waitKey()
...
otsu_thr, otsu_mask = cv2.threshold(
    img, -1, 1, cv2.THRESH_BINARY | cv2.THRESH_OTSU)

output = cv2.connectedComponentsWithStats(
    otsu_mask, connectivity, cv2.CV_32S)
...
img_color = cv2.cvtColor(otsu_mask * 255, cv2.COLOR_GRAY2BGR)

cv2.imshow('Connected components', np.hstack((img_color, colored)))
cv2.waitKey()
cv2.destroyAllWindows()