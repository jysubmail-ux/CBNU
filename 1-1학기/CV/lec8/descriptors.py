...
def video_keypoints(matcher,
    ...
    while True:
        ...
        if (cap.get(cv2.CAP_PROP_POS_FRAMES) - 1) % 40 == 0:
            ...
            matches = matcher.match(descriptors_2, descriptors_1) # 1frame 과 이후 frame을 통해 대응관계 확인
...
index_params = dict(algorithm=FLANN_INDEX_COMPOSITE, trees=16) # 해시트리
search_params = dict(checks=10)
